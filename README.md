...
| Plugin | Description |
|---|---|
| [`nerdynik-diagram-toolkit`](plugins/nerdynik-diagram-toolkit) | Mermaid and draw.io diagrams, bundling both toolchains' MCP servers. Which tool can render, validate, or actually write a file, the syntax and escaping rules that silently break a diagram in each format, and guides for org charts and project-schedule Gantt charts. |
...
| [`nerdynik-obsidian-ps-toolkit`](plugins/nerdynik-obsidian-ps-toolkit) | Obsidian vault structure for Professional Services / C&SI / SI engagements — customer/partner/practice taxonomy, tracking people across accounts, org charts kept as both a Mermaid diagram and a DrawIO file, daily summaries from call recordings and calendar data. Depends on `nerdynik-obsidian-toolkit` and `nerdynik-diagram-toolkit`; pairs with `nerdynik-plaud-toolkit`. |
